...
test("multi-page launch surface separates product, deal preparation, funding, verified cases, proof, and architecture", async () => {
  ...
  assert.match(overview, /Invoice face value<\/dt><dd>100/);
  assert.match(overview, /Advance requested<\/dt><dd>75/);
  assert.match(overview, /Contract maximum<\/dt><dd>80/);
  assert.match(overview, /Prepared, not approved or financed/);
  ...
  assert.match(overview, /0x6d17…4db66 · RECEIPT-BOUND AI/);
  assert.match(overview, /It was not escalated, funded, or settled/);
  ...
  assert.match(studio, /See an invoice's pre-AI planning ceiling in one click/);
  assert.match(studio, /id="load-sample"/);
  assert.match(studio, /Prepare the no-wallet credit memo/);
  assert.match(studio, /PRE-AI PLANNING CEILING/);
  assert.match(studio, /The contract itself hard-enforces its 80% ceiling/);
  assert.match(studio, /Prepared is not financed/);
  assert.match(studio, /Two signatures, one AI decision, and a funder's choice/);
  assert.ok(studio.indexOf('class="studio-hero-title"') < studio.indexOf('class="studio-trial guided-trial"'), "Studio title must precede the primary action");
  assert.ok(studio.indexOf('class="studio-trial guided-trial"') < studio.indexOf('class="studio-hero-guide"'), "mobile source order must put the primary action before secondary guidance");
  assert.match(studio, /<details class="studio-review-tools">/);
  assert.match(studio, /<details class="studio-advanced">/);
  assert.match(studio, /No wallet, model, financing, or blockchain transaction occurred/);
  assert.match(studio, /id="sample-document">OpenBell no-value sample invoice/);
  assert.match(css, /inter-latin-variable\.woff2/);
  assert.match(css, /\.onboarding-hero \{/);
  assert.match(css, /\.memo-boundary-callout \{/);
  ...
  assert.doesNotMatch(proof, /stricter protocol ceiling/);
  assert.match(architecture, /AI evaluates.<br \/>People authorize.<br \/>X Layer enforces/);
...
test("interactive controls use native accessible elements", async () => {
  ...
  assert.match(studio, /<option value="mainnet" selected>X Layer mainnet · real USDG<\/option>/);
  assert.match(studio, /id="deal-face-symbol">USDG</);
  assert.match(studio, /id="deal-request-symbol">USDG</);
  ...
  assert.match(studio, /Download unsigned package/);
  assert.match(studio, /<button[^>]+id="load-sample"[^>]+type="button"/);
  assert.match(studio, /<ol id="studio-journey"[^>]+aria-label="Invoice funding journey"/);
  assert.match(studio, /<article class="credit-memo guided-credit-memo" id="credit-memo" hidden aria-live="polite" tabindex="-1"/);
  assert.match(studio, /<details class="memo-technical"><summary>Show technical package details<\/summary>/);
  ...
  assert.match(script, /status = "Ready\. Nothing leaves this browser\."/);
